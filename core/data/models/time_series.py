...
class BuiltSerie(TimeSerie):
    ...
    def update(self, verbose=False, next_timestamp=None):
        """
        ...
        if next_timestamp is None: 
            if self.timestamps[-1] > 0:
                next_timestamp = int( self.timestamps[-1] + self.duration )
            else:
                next_timestamp = int( (self.dependencies[0].timestamps[-1]) / self.duration ) * self.duration
        
        # if the time series is already up to date there is nothing to do
        if next_timestamp >= int( (time.time() * 1000) / self.duration ) * self.duration:
            return
        ...
        for parent in self.dependencies:
            ...
            if parent.timestamps[-1] < last_desired:
                if verbose:
                    print(70*"_")
                    print( datetime.datetime.fromtimestamp(next_timestamp/1000), ": failed computation of ", " local", self.symbol, self.serie_type, self.duration)
                    print( f"Desired parent timestamp: {last_desired} ; Last parent timestamp: {parent.timestamps[-1][0]}")
                miss_data = True
                break
                ...
                if verbose:
                    print(70*"_")
                    print( datetime.datetime.fromtimestamp(next_timestamp/1000), ": failed computation of ", " local", self.symbol, self.serie_type, self.duration)
                    print( f"Desired parent timestamp: {last_desired} ; Last parent timestamp: {self.dependencies[0].timestamps[-1][0]}")
                self.update(verbose = verbose, next_timestamp = parent.timestamps[-1][0])
                verbose = False
                break
        ...
        if miss_data:
            ...
            print(70*"_")
            print( datetime.datetime.fromtimestamp(next_timestamp/1000), ": successful computation of ", " local", self.symbol, self.serie_type, self.duration)
...
class ThirdPartySerie(TimeSerie):
    ...
    def update(self, new_values, new_timestamp):
        ...
        None
        """
        ### achtung: np.roll does not work with mpi4py shared memory
        #self.data = np.roll(self.data, -1, axis=0)
